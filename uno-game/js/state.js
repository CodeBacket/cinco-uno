...
const ALLOWED_PHASE_TRANSITIONS = Object.freeze({
  [GAME_PHASES.MENU]: new Set([GAME_PHASES.PLAYING]),
  [GAME_PHASES.PLAYING]: new Set([GAME_PHASES.ROUND_OVER, GAME_PHASES.GAME_OVER]),
  [GAME_PHASES.ROUND_OVER]: new Set([GAME_PHASES.MENU, GAME_PHASES.GAME_OVER]),
  [GAME_PHASES.GAME_OVER]: new Set([GAME_PHASES.MENU]),
});
...
function canTransitionPhase(fromPhase, toPhase) {
  ...
  return Boolean(allowedTargets && allowedTargets.has(toPhase));
}
