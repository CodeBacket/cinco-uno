...
11. **Events array**: push structured events for UI (`{ type, payload }`) for each mutation and reuse canonical `EVENT_TYPES` exported from `uno-game/js/state.js` (`PHASE_CHANGED`, `TURN_CHANGED`, `CARD_PLAYED`, `CARD_DRAWN`, `WILD_COLOR_CHOSEN`, `PENALTY_APPLIED`, `DISCARD_RESHUFFLED`, `ROUND_WON`, `ERROR`).
