...
11. **Events array**: push structured events for UI (`{ type, payload }`) for each mutation, with `type` values sourced from canonical `EVENT_TYPES` in `uno-game/js/state.js` (for example `PHASE_CHANGED`, `TURN_CHANGED`, `CARD_PLAYED`, `CARD_DRAWN`, `ROUND_WON`, `ERROR`).
