...
11. **Events array**: push structured events for UI (`{ type, payload }`) for each mutation, using `EVENT_TYPES` exported by `uno-game/js/state.js` as the canonical event vocabulary.
