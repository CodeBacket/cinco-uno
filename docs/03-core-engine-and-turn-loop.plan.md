...
11. **Events array**: push structured events for UI (`{ type, payload }`) for each mutation, and source `type` values from `EVENT_TYPES` in `uno-game/js/state.js` (for example `TURN_CHANGED`, `CARD_PLAYED`, `CARD_DRAWN`, `ROUND_WON`, `ERROR`).
